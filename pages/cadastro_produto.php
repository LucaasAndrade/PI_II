...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $url_imagem = $base_url . "uploads/" . basename($nome);

    // Mover o arquivo de imagem carregado para o diretório de destino
    if (!empty($imagem) && move_uploaded_file($_FILES['imagem']['tmp_name'], $target_file)) {
    } else {
        echo "Falha ao carregar a imagem";
    }

    try {
        $sql = "INSERT INTO PRODUTO (PRODUTO_NOME, PRODUTO_DESC, PRODUTO_PRECO, CATEGORIA_ID, PRODUTO_ATIVO) VALUES (:nome,
        :descricao, :preco, :categoria_id, :ativo)";
        ...
        foreach ($imagens as $ordem => $url_imagem) {
            ...
            $stmt_imagem->bindParam(':ordem_imagem', $ordem_imagem, PDO::PARAM_STR);
    ...
    <body>
        ...
        <section class="form__container container mt-5">
            <div class="back__container">
                <a href="painel_adm.php" class="btn btn-primary"><i class="fa-solid fa-arrow-rotate-left"></i></i> Voltar</a>
            </div>
            <form action="cadastro_produto.php" method="post" enctype="multipart/form-data" class="form__products">
                <div class="mb-3">
